...
if __name__ == '__main__':
    ...
    with open("rmtt_all_drone.launch","w") as f:
        ...
        for idx,sn  in enumerate(SN_LIST):
            ...
            f.write(f'\t<group ns="/sunray_rmtt/rmtt_{idx +1}">\n')
            ...
            f.write(f'\t\t<node pkg="sunray_rmtt" type="rmtt_control_node" name="rmtt_control_node_{idx+1}" output="screen">\n')
            f.write(f'\t\t\t<param name="uav_id" value="{idx+1}" />\n')
            f.write(f'\t\t\t<param name="uav_ip" value="{ip}" />\n')
            f.write(f'\t\t\t<rosparam command="load" file="$(find sunray_rmtt)/launch/rmtt_params.yaml" />\n')
            f.write(f'\t\t</node>\n')
            f.write(f'\t</group>\n')
            f.write('\n')
