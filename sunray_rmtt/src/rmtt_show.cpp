...
int main(int argc, char **argv)
{
    ...
    // 【订阅】无人机状态数据
    rmtt_state_sub = nh.subscribe<sunray_msgs::rmtt_state>("/sunray_rmtt" + agent_name + "/rmtt_state", 1, rmtt_state_cb);
    // 【订阅】无人机orca状态 回传地面站
    rmtt_orca_state_sub = nh.subscribe<sunray_msgs::rmtt_orca>("/sunray_rmtt" + agent_name + "/rmtt_orca", 1, rmtt_orca_state_cb);
    // 【发布】无人机的目标点
    rmtt_goal_pub = nh.advertise<geometry_msgs::Point>("/sunray_rmtt" + agent_name + "/goal_point", 1);
    // 【发布】地面站指令
    station_cmd_pub = nh.advertise<sunray_msgs::station_cmd>("/sunray_rmtt" + agent_name + "/station_cmd", 1);
    // 【发布】文字提示消息（回传至地面站显示）
