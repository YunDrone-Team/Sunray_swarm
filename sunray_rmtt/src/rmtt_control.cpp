...
void RMTT_CONTROL::init(ros::NodeHandle& nh)
{
    ...
    // 【订阅】地面站指令 地面站 -> 本节点
    station_cmd_sub = nh.subscribe<sunray_msgs::station_cmd>("/sunray_rmtt/station_cmd", 1, &RMTT_CONTROL::station_cmd_cb, this);   
    // 【订阅】地面站指令 地面站 -> 本节点(show专用)
    station_cmd_sub2 = nh.subscribe<sunray_msgs::station_cmd>("/sunray_rmtt" + agent_name + "/station_cmd", 1, &RMTT_CONTROL::station_cmd_cb2, this);
    // 【订阅】rmtt电池的数据 rmtt_driver -> 本节点
    battery_sub = nh.subscribe<std_msgs::Float32>("/sunray_rmtt" + agent_name + "/battery", 1, &RMTT_CONTROL::battery_cb, this);  
    // 【订阅】orca_cmd_vel rmtt_orca -> 本节点
    orca_cmd_sub = nh.subscribe<geometry_msgs::Twist>("/sunray_rmtt" + agent_name + "/orca_cmd_vel", 1, &RMTT_CONTROL::orca_cmd_vel_cb, this);  
    // 【发布】无人机orca状态 rmtt_orca -> 本节点
    rmtt_orca_state_sub = nh.subscribe<sunray_msgs::rmtt_orca>("/sunray_rmtt" + agent_name + "/rmtt_orca_state", 1, &RMTT_CONTROL::rmtt_orca_state_cb, this);    
    // 【发布】无人机控制指令（机体系，单位：米/秒，Rad/秒）
    rmtt_cmd_pub = nh.advertise<geometry_msgs::Twist>("/sunray_rmtt" + agent_name + "/cmd_vel", 1); 
    // 【发布】无人机起飞指令 本节点 -> rmtt_driver
    takeoff_pub = nh.advertise<std_msgs::Empty>("/sunray_rmtt" + agent_name + "/takeoff", 1); 
    // 【发布】无人机降落指令 本节点 -> rmtt_driver
    land_pub = nh.advertise<std_msgs::Empty>("/sunray_rmtt" + agent_name + "/land", 1); 
    // 【发布】led灯 本节点 -> rmtt_driver
    led_pub = nh.advertise<std_msgs::ColorRGBA>("/sunray_rmtt" + agent_name + "/led", 1);
    // 【发布】字符显示 本节点 -> rmtt_driver
    mled_pub = nh.advertise<std_msgs::String>("/sunray_rmtt" + agent_name + "/mled", 1);
    // 【发布】rmtt_state 本节点 -> 地面站
    rmtt_state_pub = nh.advertise<sunray_msgs::rmtt_state>("/sunray_rmtt" + agent_name + "/rmtt_state", 1); 
    // 【发布】无人机位置(带图标) 本节点 -> RVIZ
    rmtt_mesh_pub = nh.advertise<visualization_msgs::Marker>("/sunray_rmtt" + agent_name + "/rmtt_mesh", 1);
    // 【发布】运动轨迹  本节点 -> RVIZ
    rmtt_trajectory_pub = nh.advertise<nav_msgs::Path>("/sunray_rmtt" + agent_name + "/rmtt_trajectory", 1);
    // 【发布】文字提示消息  本节点 -> 地面站
    text_info_pub = nh.advertise<std_msgs::String>("/sunray_rmtt/text_info", 1);
    // 【发布】无人机orca状态 本节点 -> 地面站
    rmtt_orca_state_pub = nh.advertise<sunray_msgs::rmtt_orca>("/sunray_rmtt" + agent_name + "/rmtt_orca", 1);    
    // 【发布】目标点marker 本节点 -> RVIZ
    goal_point_pub = nh.advertise<visualization_msgs::Marker>("/sunray_rmtt" + agent_name + "/goal_point_rviz", 1);
...
void RMTT_CONTROL::timercb_trajectory(const ros::TimerEvent &e)
{
    ...
    rmtt_marker.mesh_resource = std::string("package://sunray_rmtt/meshes/hummingbird.mesh");
