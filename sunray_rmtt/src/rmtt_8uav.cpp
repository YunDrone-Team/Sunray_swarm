...
int main(int argc, char **argv)
{
    ...
    // 【发布】地面站指令
    station_cmd_pub = nh.advertise<sunray_msgs::station_cmd>("/sunray_rmtt/station_cmd", 1);
    // 【发布】文字提示消息（回传至地面站显示）
    ...
    for(int i = 0; i < rmtt_num; i++) 
    {
        ...
        // 【订阅】无人机状态数据
        rmtt_state_sub[i] = nh.subscribe<sunray_msgs::rmtt_state>("/sunray_rmtt" + agent_name + "/rmtt_state", 1, boost::bind(&rmtt_state_cb,_1,i));
        // 【订阅】无人机orca状态 回传地面站
		rmtt_orca_state_sub[i] = nh.subscribe<sunray_msgs::rmtt_orca>("/sunray_rmtt" + agent_name + "/rmtt_orca", 1, boost::bind(&rmtt_orca_state_cb,_1,i));
        // 【发布】无人机的目标点
        rmtt_goal_pub[i] = nh.advertise<geometry_msgs::Point>("/sunray_rmtt" + agent_name + "/goal_point", 1);
        // 【发布】无人机起飞指令
		takeoff_pub[i] = nh.advertise<std_msgs::Empty>("/sunray_rmtt" + agent_name + "/takeoff", 1); 
        // 【发布】无人机降落指令
		land_pub[i] = nh.advertise<std_msgs::Empty>("/sunray_rmtt" + agent_name + "/land", 1); 
    }
