...
void RMTT_SIM::init(ros::NodeHandle& nh)
{
    ...
    nh.param<float>("agent_height", agent_height, 0);
    nh.param<double>("init_pos_x", agent_pos.pose.position.x, 0.0);
    nh.param<double>("init_pos_y", agent_pos.pose.position.y, 0.0);
    nh.param<double>("init_pos_z", agent_pos.pose.position.z, 0.0);
    nh.param<double>("init_yaw", agent_yaw, 0.0);

    string agent_name = "/rmtt_" + std::to_string(agent_id);
    // 【订阅】地面站指令 地面站 -> 本节点 
    rmtt_cmd_sub = nh.subscribe<sunray_msgs::rmtt_cmd>("/sunray_swarm" + agent_name + "/rmtt_cmd", 1, &RMTT_SIM::rmtt_cmd_cb, this);  
    // 【订阅】无人机cmd数据
    rmtt_cmd_vel_sub = nh.subscribe<geometry_msgs::Twist>("/sunray_swarm" + agent_name + "/cmd_vel", 1, &RMTT_SIM::rmtt_cmd_vel_cb, this);
    // 【订阅】无人机orca算法状态
    orca_state_sub = nh.subscribe<sunray_msgs::orca_state>("/sunray_swarm" + agent_name + "/orca_state", 1, &RMTT_SIM::agent_orca_state_cb, this);
    // 【发布】无人机位置&速度数据（仿真中模拟）
...
void RMTT_SIM::rmtt_cmd_cb(const sunray_msgs::rmtt_cmd::ConstPtr& msg)
{
...
void RMTT_SIM::agent_orca_state_cb(const sunray_msgs::orca_state::ConstPtr& msg)
{
    agent_orca_state = *msg;
}

void RMTT_SIM::rmtt_cmd_vel_cb(const geometry_msgs::Twist::ConstPtr& msg)
{
    cmd_vel = *msg;
}

// 从(roll,pitch,yaw)创建四元数  by a 3-2-1 intrinsic Tait-Bryan rotation sequence
geometry_msgs::Quaternion RMTT_SIM::ros_quaternion_from_rpy(double roll, double pitch, double yaw)
{
