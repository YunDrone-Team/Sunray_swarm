...
void RMTT_CONTROL::init(ros::NodeHandle& nh)
{
    ...
    // 【订阅】地面站指令 地面站 -> 本节点
    rmtt_cmd_sub = nh.subscribe<sunray_msgs::rmtt_cmd>("/sunray_swarm" + agent_name + "/rmtt_cmd", 1, &RMTT_CONTROL::rmtt_cmd_cb, this);
    ...
    // 【订阅】orca_cmd_vel orca -> 本节点
    orca_cmd_sub = nh.subscribe<geometry_msgs::Twist>("/sunray_swarm" + agent_name + "/orca_cmd_vel", 1, &RMTT_CONTROL::orca_cmd_vel_cb, this);    
    // 【订阅】无人机orca状态 orca -> 本节点
    rmtt_orca_state_sub = nh.subscribe<sunray_msgs::orca_state>("/sunray_swarm" + agent_name + "/agent_orca_state", 1, &RMTT_CONTROL::rmtt_orca_state_cb, this);    
    ...
    rmtt_state.agent_type = agent_type;
    rmtt_state.agent_id = agent_id;
    rmtt_state.agent_ip = agent_ip;  
    rmtt_state.connected = false;
    rmtt_state.odom_valid = false;
    rmtt_state.pos[0] = 0.0;
    rmtt_state.pos[1] = 0.0;
    rmtt_state.pos[2] = agent_height;
    ...
    rmtt_state.attitude_q.x = 0.0;
    rmtt_state.attitude_q.y = 0.0;
    rmtt_state.attitude_q.z = 0.0;
    rmtt_state.attitude_q.w = 1.0;
    rmtt_state.battery = -1.0;
    rmtt_state.control_state = CONTROL_STATE::INIT;
...
void RMTT_CONTROL::mainloop()
{
    ...
    case CONTROL_STATE::VEL_CONTROL:
        ...
        // 发布控制指令
        rmtt_cmd_pub.publish(desired_vel);        
        break;

    case CONTROL_STATE::ORCA_MODE:
        if(rmtt_orca_state.arrived_goal)
        {
            desired_position.x = rmtt_orca_state.goal[0];
            desired_position.y = rmtt_orca_state.goal[1];
    ...
    case CONTROL_STATE::RETURN_HOME:
        if(rmtt_orca_state.arrived_goal)
        {
            control_state = CONTROL_STATE::LAND;
        }else
        {
            // orca control
            orca_control();
...
void RMTT_CONTROL::orca_control()
{
    ...
    // 控制指令计算：使用简易P控制 - Z
    cmd_vel_orca_body.linear.z = (agent_height - rmtt_state.pos[2]) * rmtt_control_param.Kp_z;
    // 控制指令计算：使用简易P控制 - YAW
    cmd_vel_orca_body.angular.x = 0.0;
    cmd_vel_orca_body.angular.y = 0.0;
...
{
    desired_position.x = rmtt_state.pos[0];
    desired_position.y = rmtt_state.pos[1];
...
void RMTT_CONTROL::pos_control(geometry_msgs::Point pos_ref, double yaw_ref)
{
    ...
    float cmd_enu[2];
    // 控制指令计算：使用简易P控制 - XY
    cmd_enu[0] = (pos_ref.x - rmtt_state.pos[0]) * rmtt_control_param.Kp_xy;
    cmd_enu[1] = (pos_ref.y - rmtt_state.pos[1]) * rmtt_control_param.Kp_xy;
    rotation_yaw(rmtt_state.att[2], cmd_body, cmd_enu);             
    desired_vel.linear.x = cmd_body[0];
    desired_vel.linear.y = cmd_body[1];
    // 控制指令计算：使用简易P控制 - Z
    desired_vel.linear.z = (pos_ref.z - rmtt_state.pos[2]) * rmtt_control_param.Kp_z;
    // YAW误差计算
    double yaw_error = get_yaw_error(desired_yaw, rmtt_state.att[2]);
    // 控制指令计算：使用简易P控制 - YAW
...
void RMTT_CONTROL::rmtt_cmd_cb(const sunray_msgs::rmtt_cmd::ConstPtr& msg)
{
    ...
    current_rmtt_cmd = *msg; 
    switch(msg->control_state) 
    {
        case CONTROL_STATE::INIT:
            control_state = CONTROL_STATE::INIT;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: INIT!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::TAKEOFF:
            control_state = CONTROL_STATE::TAKEOFF;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: TAKEOFF!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::LAND:
            control_state = CONTROL_STATE::LAND;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: LAND!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::HOLD:
            set_desired_position();
            control_state = CONTROL_STATE::HOLD;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: HOLD!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::POS_CONTROL:
            control_state = CONTROL_STATE::POS_CONTROL;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: POS_CONTROL!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::VEL_CONTROL:
            control_state = CONTROL_STATE::VEL_CONTROL;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: VEL_CONTROL!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::ORCA_MODE:
            control_state = CONTROL_STATE::ORCA_MODE;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: ORCA_MODE!";
            cout << BLUE << text_info.data << TAIL << endl;
            break;
        case CONTROL_STATE::RETURN_HOME:
            control_state = CONTROL_STATE::RETURN_HOME;
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: RETURN_HOME!";
            cout << BLUE << text_info.data << TAIL << endl;
            sleep(1.0);
            rmtt_orca_state.arrived_goal = false;
            break;
        default:
            text_info.data = node_name + ": rmtt_" + to_string(agent_id) + " Get rmtt_cmd: Wrong!";
...
void RMTT_CONTROL::timercb_debug(const ros::TimerEvent &e)
{
    ...
    cout << GREEN << "RMTT_pos [X Y Z] : " << rmtt_state.pos[0] << " [ m ] " << rmtt_state.pos[1] << " [ m ] " << rmtt_state.pos[2] << " [ m ] " << TAIL << endl;
    cout << GREEN << "RMTT_vel [X Y Z] : " << rmtt_state.vel[0] << " [m/s] " << rmtt_state.vel[1] << " [m/s] " << rmtt_state.vel[2] << " [m/s] " << TAIL << endl;
    cout << GREEN << "RMTT_att [R P Y] : " << rmtt_state.att[0] * 180 / M_PI << " [deg] " << rmtt_state.att[1] * 180 / M_PI << " [deg] " << rmtt_state.att[2] * 180 / M_PI << " [deg] " << TAIL << endl;
    ...
    else if (control_state == CONTROL_STATE::TAKEOFF)
    {
        cout << GREEN << "CONTROL_STATE : [ TAKEOFF ]" << TAIL << endl;
    }
    else if (control_state == CONTROL_STATE::LAND)
    {
...
void RMTT_CONTROL::timercb_state(const ros::TimerEvent &e)
{
    ...
    rmtt_state.control_state = control_state;
    agent_state_pub.publish(rmtt_state);
}
...
void RMTT_CONTROL::timercb_rviz(const ros::TimerEvent &e)
{
    ...
    rmtt_marker.scale.x = 0.8;  
    rmtt_marker.scale.y = 0.8;  
    rmtt_marker.scale.z = 0.8;  
