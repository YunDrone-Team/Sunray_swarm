    ...
    while (ros::ok())
    {
        ...
        switch(formation_state)
        {
            ...
            case FORMATION_STATE::K:
                ...
                {
                    sleep(formation_time);                      // 等待队形时间
                    formation_state = FORMATION_STATE::V;      // 转换到O2形队形
                    pub_goal_once = false;                      // 重置标志
                }
                break;
...
void setup_show_goals()
{
    ...
    // goal_V[7].x = -2.0; goal_V[7].y = -4.3; // 底部右端
        ...
        // 字母 N
    goal_N[0].x = -2.08; goal_N[0].y = -1.44;   // 顶部左端
    goal_N[1].x = -2.04; goal_N[1].y = 0.32;  // 底部左端


    // 字母 O
    goal_O[0].x = 2.00;  goal_O[0].y = -1.48;  // 上部中点
    goal_O[1].x = 2.02;  goal_O[1].y = 0.31; // 下部中点

    // 字母 K
    goal_K[0].x = 2.02; goal_K[0].y = -0.32;  // 上部交点
    goal_K[1].x = 2.02; goal_K[1].y = 1.42; // 下部交点

    // 字母 V
    goal_V[0].x = -2.07;  goal_V[0].y = 0.08;  // 左上点
    goal_V[1].x = -1.98;  goal_V[1].y = 1.50; // 右上点
