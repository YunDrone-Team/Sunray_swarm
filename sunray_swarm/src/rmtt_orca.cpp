...
void RMTT_ORCA::init(ros::NodeHandle& nh)
{
    ...
    // 初始化订阅器和发布器
    // 【订阅】地面站指令
    station_cmd_sub = nh.subscribe<sunray_msgs::station_cmd>("/sunray_swarm/station_cmd", 1, &RMTT_ORCA::station_cmd_cb, this);
    // 【发布】文字提示消息（回传至地面站显示）
    ...
    for(int i = 0; i < rmtt_num; i++) 
    {
        ...
        // 【订阅】无人机状态数据
        rmtt_state_sub[i] = nh.subscribe<sunray_msgs::rmtt_state>("/sunray_swarm" + agent_name + "/rmtt_state", 1, boost::bind(&RMTT_ORCA::rmtt_state_cb,this ,_1,i));
        // 【订阅】无人机的目标点
        rmtt_goal_sub[i] = nh.subscribe<geometry_msgs::Point>("/sunray_swarm" + agent_name + "/goal_point", 1, boost::bind(&RMTT_ORCA::rmtt_goal_cb,this ,_1,i));
        // 【发布】无人机ORCA控制指令（ENU系，单位：米/秒，Rad/秒）
		orca_cmd_pub[i] = nh.advertise<geometry_msgs::Twist>("/sunray_swarm" + agent_name + "/orca_cmd_vel", 1); 
        // 【发布】无人机orca状态 回传地面站
		rmtt_orca_state_pub[i] = nh.advertise<sunray_msgs::rmtt_orca>("/sunray_swarm" + agent_name + "/rmtt_orca_state", 1);
        target_name = "/tianbot_0" + std::to_string(i+1);
        // 【订阅】订阅rmtt动捕的数据(位置+速度) vrpn -> 本节点 
        mocap_target_pos_sub[i] = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node"+ target_name + "/pose", 1, boost::bind(&RMTT_ORCA::mocap_target_pos_cb, this,_1,i));
    }
...
void RMTT_ORCA::mocap_target_pos_cb(const geometry_msgs::PoseStamped::ConstPtr& msg, int i)
{
    if(!track_mode)
    {
        return;
    ...
    arrived_goal[i] = false;
    arrived_all_goal = false;    
    rmtt_goal[i].x = msg->pose.position.x;
    rmtt_goal[i].y = msg->pose.position.y;
    // 高度固定
