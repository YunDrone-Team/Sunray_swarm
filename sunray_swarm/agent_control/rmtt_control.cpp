...
void RMTT_CONTROL::init(ros::NodeHandle& nh)
{
    ...
    // 【订阅】地面站指令 地面站 -> 本节点
    agent_cmd_sub = nh.subscribe<sunray_msgs::agent_cmd>("/sunray_swarm/" + agent_name + "/agent_cmd", 100, &RMTT_CONTROL::agent_cmd_cb, this);
...
void RMTT_CONTROL::timercb_rviz(const ros::TimerEvent &e)
{
    ...
    //  |----坐标系 ID
    tfs.child_frame_id = "rmtt_" + std::to_string(agent_id) + "/base_link"; //子坐标系，无人机的坐标系
